            ...
            <!-- Navigation -->
              ...
              >
                ...
                <div>
                    ...
                    <!-- User Manual -->
                    <Link 
                        v-if="hasPermission('view_user_manual')"
                        :href="route('user-manual.index')" 
                        @mouseenter="handleMouseEnter($event, $t('nav.user_manual'))"
                        @mouseleave="handleMouseLeave"
                        :class="[
                            'group flex items-center rounded-lg transition-all duration-200 mt-1',
                            isSidebarCollapsed ? 'justify-center p-2' : 'px-3 py-2.5',
                            $page.url.includes('/user-manual') 
                                ? 'bg-primary/10 text-primary font-medium sidebar-active-indicator sidebar-active-glow' 
                                : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/50 hover:text-gray-900 dark:hover:text-white'
                        ]"
                    >
                        ...
                        <span class="text-sm" v-if="!isSidebarCollapsed">{{ $t('nav.user_manual') }}</span>
                    </Link>
                </div>
                            ...
                            <!-- Sub-Sub Items: Orders -->
                             <div v-show="openMenus['operations.orders'] && !isSidebarCollapsed" class="pl-9 space-y-0.5">
                                ...
                                >
                                    ...
                                    <span v-if="!isSidebarCollapsed">{{ $t('inventory.reminders', 'Reminders') }}</span>
                                </Link>
                        </div>
