<template>
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans overflow-hidden py-4 px-6 flex flex-col">
        ...
        <div class="flex items-center justify-between mb-6 border-b border-gray-200 dark:border-gray-800 pb-4">
            <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Order Status Manager <span class="text-sm font-normal text-gray-500 ml-2">(Click cards to advance status)</span></h1>
                ...
                <div class="flex-1 p-6 overflow-y-auto custom-scrollbar bg-gray-50/50 dark:bg-transparent">
                    <div class="grid grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        >
                            ...
                            <span class="text-xs text-gray-500 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">Click to Mark Ready</span>
                        </div>
                ...
                <div class="flex-1 p-6 overflow-y-auto custom-scrollbar bg-gray-50/50 dark:bg-transparent">
                     <div class="grid grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        >
                            ...
                            <span class="text-xs text-green-600 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">Click to Complete (Pickup)</span>
                        </div>
...
const updateStatus = (orderId: string, action: 'mark_ready' | 'mark_completed') => {
    if (processingId.value) return;
    ...
    router.post(route('orders.status-screen.update-state'), {
        order_id: orderId,
        action: action
    }, {
        preserveScroll: true,
