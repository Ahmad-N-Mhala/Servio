<template>
    <MainLayout>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" :dir="isRtl ? 'rtl' : 'ltr'">
            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $t('integrations.delivery_providers') }}</h1>
                <p class="mt-2 text-gray-500 dark:text-gray-400">{{ $t('integrations.subtitle') }}</p>
            </div>


            <!-- Connected Integrations -->
            <div v-if="connectedProviders.length > 0" class="mb-12">
                <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-6">{{ $t('integrations.connected_integrations') }}</h2>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div v-for="provider in connectedProviders" :key="provider.id" class="glass-card p-6 rounded-2xl relative overflow-hidden transition-all duration-300 hover:shadow-lg border border-green-200 dark:border-green-900 bg-green-50/30 dark:bg-green-900/10">
                        ...
                        <div class="absolute top-4 right-4">
                            ...
                            >
                                {{ $t('integrations.integration_error') }}
                                <div class="absolute bottom-full right-0 mb-2 hidden group-hover:block w-48 p-2 bg-gray-900 text-white text-xs rounded shadow-lg z-20">
                                    {{ provider.integration_error_message || $t('integrations.integration_error_msg') }}
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-3 mt-2">
                                     <button @click="editIntegration(provider.id)" class="text-xs font-medium text-primary hover:text-primary-dark hover:underline">
                                        {{ $t('integrations.configure_settings') }}
                                    </button>
                        ...
                        <div v-if="activeProvider === provider.id" class="space-y-4 animate-fade-in border-t border-gray-100 dark:border-gray-700 pt-4 mt-4">
                            <!-- Helper Text -->
                             <div class="bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 p-3 rounded-lg text-xs" v-if="provider.webhook_url_template">
                                <p class="font-bold mb-1">{{ $t('integrations.webhook_configuration') }}:</p>
                                <p>{{ $t('integrations.copy_url_to_dashboard', { provider: provider.name }) }}</p>
                            ...
                            <div class="space-y-4">
                                ...
                                <div v-if="provider.requires_store_id">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.store_id') }}</label>
                                ...
                                <div v-if="provider.requires_api_key">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.api_key') }}</label>
                                 ...
                                 <div v-if="provider.requires_api_secret">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.api_secret') }}</label>
                                ...
                                <div v-if="provider.requires_client_id">
                                 ...
                                 <div v-if="provider.requires_username">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.username') }}</label>
                                 ...
                                 <div v-if="provider.requires_password">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.password') }}</label>
                                ...
                                <div v-if="provider.requires_webhook_secret">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.webhook_secret') }}</label>
                                 ...
                                 <div class="flex items-center gap-2 mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                                    ...
                                    >
                                    <label :for="`enable-${provider.id}`" class="text-sm font-medium text-gray-900 dark:text-white cursor-pointer select-none">{{ $t('integrations.enable_integration') }}</label>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-3 pt-4">
                                ...
                                >
                                    {{ $t('integrations.update_configuration') }}
                                </Button>
                                 <Button 
                             ...
                             <div class="pt-4 border-t border-gray-100 dark:border-gray-700">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    ...
                                    >
                                        ...
                                        {{ $t('integrations.force_push_menu') }}
                                    </Button>
                                </div>
                                <p class="text-[10px] text-gray-400 mt-2 text-center" v-if="getIntegration(provider.id).last_menu_push_at">
                                    {{ $t('integrations.last_synced', { time: new Date(getIntegration(provider.id).last_menu_push_at).toLocaleString() }) }}
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div>
                 <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-6">{{ $t('integrations.available_integrations') }}</h2>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                         ...
                         <div v-if="activeProvider === provider.id" class="space-y-4 animate-fade-in border-t border-gray-100 dark:border-gray-700 pt-4 mt-4">
                            <!-- Helper Text -->
                             <div class="bg-yellow-50 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-200 p-3 rounded-lg text-xs mb-4">
                                <p class="font-bold">{{ $t('integrations.setup_instructions') }}:</p>
                                <p>{{ $t('integrations.enter_credentials_msg', { provider: provider.name }) }}</p>
                            </div>
                             ...
                             <div class="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg text-xs mb-4" v-if="provider.webhook_url_template">
                                <p class="font-bold mb-1 text-gray-700 dark:text-gray-300">{{ $t('integrations.webhook_url_label') }}:</p>
                            ...
                            <div class="space-y-4">
                                 <!-- Store ID -->
                                <div v-if="provider.requires_store_id">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.store_id') }} <span class="text-red-500">*</span></label>
                                    <Input v-model="forms[provider.id].store_id" :error="forms[provider.id].errors.store_id" type="text" placeholder="e.g. 15482" />
                                </div>
                                <div v-if="provider.requires_api_key">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.api_key') }} <span class="text-red-500">*</span></label>
                                ...
                                <!-- Add other fields as necessary... using same v-if logic -->
                                 <div v-if="provider.requires_api_secret">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.api_secret') }} <span class="text-red-500">*</span></label>
                                    <Input v-model="forms[provider.id].api_secret" :error="forms[provider.id].errors.api_secret" type="password" />
                                </div>
                                <div v-if="provider.requires_client_id">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.client_id') }} <span class="text-red-500">*</span></label>
                                    <Input v-model="forms[provider.id].client_id" :error="forms[provider.id].errors.client_id" type="text" />
                                </div>
                                <div v-if="provider.requires_client_secret">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.client_secret') }} <span class="text-red-500">*</span></label>
                                    <Input v-model="forms[provider.id].client_secret" :error="forms[provider.id].errors.client_secret" type="password" />
                                </div>
                                <div v-if="provider.requires_username">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.username') }} <span class="text-red-500">*</span></label>
                                    <Input v-model="forms[provider.id].username" :error="forms[provider.id].errors.username" type="text" />
                                </div>
                                <div v-if="provider.requires_password">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.password') }} <span class="text-red-500">*</span></label>
                                    <Input v-model="forms[provider.id].password" :error="forms[provider.id].errors.password" type="password" />
                                </div>
                                <div v-if="provider.requires_webhook_secret">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ $t('integrations.webhook_secret') }}</label>
                                ...
                                <div class="flex items-center gap-2 mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                                    <input 
                                        ...
                                        class="rounded border-gray-300 text-primary focus:ring-primary w-5 h-5"
                                    >
                                    <label :for="`enable-new-${provider.id}`" class="text-sm font-medium text-gray-900 dark:text-white cursor-pointer select-none">{{ $t('integrations.enable_immediately') }}</label>
                                </div>
                            </div>
            ...
            <div class="mt-12 bg-blue-50 dark:bg-blue-900/10 rounded-2xl p-6 border border-blue-100 dark:border-blue-900/20">
                <h3 class="text-lg font-bold text-blue-900 dark:text-blue-100 mb-4">{{ $t('integrations.how_to_integrate') }}</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div>
                        <h4 class="font-semibold text-blue-800 dark:text-blue-200 mb-2">{{ $t('integrations.get_credentials') }}</h4>
                        <p class="text-sm text-blue-700 dark:text-blue-300">{{ $t('integrations.get_credentials_desc') }}</p>
                    </div>
                    <div>
                        <h4 class="font-semibold text-blue-800 dark:text-blue-200 mb-2">{{ $t('integrations.enter_details') }}</h4>
                        <p class="text-sm text-blue-700 dark:text-blue-300">{{ $t('integrations.enter_details_desc') }}</p>
                    </div>
                     <div>
                        <h4 class="font-semibold text-blue-800 dark:text-blue-200 mb-2">{{ $t('integrations.webhooks_config') }}</h4>
                        <p class="text-sm text-blue-700 dark:text-blue-300">{{ $t('integrations.webhooks_config_desc') }}</p>
                    </div>
                </div>
            </div>
        </div>
...
const disconnectIntegration = (providerId: string) => {
    if (!confirm(t('integrations.disconnect_confirm'))) {
        return;
    }
