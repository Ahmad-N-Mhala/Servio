        ...
        <div class="py-6">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-8">
                    ...
                    <!-- Usage Warning -->
                    <div v-if="provider.integrations_count > 0" class="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex items-start">
                        ...
                        <div>
                            <p class="text-sm font-medium text-yellow-800">
                                This provider is currently being used by {{ provider.integrations_count }} restaurant(s)
                            </p>
                            <p class="text-xs text-yellow-700 mt-1">
                                Changes to requirements may affect existing integrations
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <form @submit.prevent="submit">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <input 
                                            ...
                                            required
                                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                                            placeholder="e.g., talabat, noon"
                                        />
                                        <p class="mt-1 text-xs text-gray-500">Used in integrations - change carefully</p>
                                ...
                                <div class="mt-6">
                                    ...
                                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-blue-500 transition-colors bg-gray-50 hover:bg-white">
                                        <div class="space-y-1 text-center">
                                            <div v-if="!logoPreview && !form.logo_url" class="mx-auto h-12 w-12 text-gray-400">
                                                <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
                                                    <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                                            </div>
                                            <img v-else :src="logoPreview || form.logo_url" class="mx-auto h-24 w-24 object-contain rounded-lg shadow-sm" />
                        ...
                        <div class="px-8 py-6 bg-gray-50 border-t border-gray-200 flex justify-between">
                            <button 
                                ...
                                class="px-6 py-3 border border-red-300 text-red-700 rounded-lg font-medium hover:bg-red-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                :title="provider.integrations_count > 0 ? 'Cannot delete provider with active integrations' : ''"
                            >
                                Delete Provider
                            </button>
...
const form = useForm({
    ...
    logo_url: props.provider.logo_url || '',
    api_documentation_url: props.provider.api_documentation_url || '',
    requires_api_key: Boolean(props.provider.requires_api_key),
    requires_api_secret: Boolean(props.provider.requires_api_secret),
    requires_client_id: Boolean(props.provider.requires_client_id),
    requires_client_secret: Boolean(props.provider.requires_client_secret),
    requires_username: Boolean(props.provider.requires_username),
    requires_password: Boolean(props.provider.requires_password),
    requires_store_id: Boolean(props.provider.requires_store_id),
    requires_webhook_secret: Boolean(props.provider.requires_webhook_secret),
    webhook_url_template: props.provider.webhook_url_template || '',
...
const submit = () => {
    ...
    // Using Inertia useForm with spoofed PUT method and forced FormData
    // Since Inertia useForm doesn't support merging data in post easily, we manually invoke
    
    form.transform((data) => ({
        ...data,
        _method: 'PUT',
    })).post(route('admin.delivery-providers.update', props.provider.id), {
        forceFormData: true,
        onError: (err: any) => {
            errors.value = err;
...
const deleteProvider = () => {
    if (confirm('Are you sure you want to delete this delivery provider? This action cannot be undone.')) {
        router.delete(route('admin.delivery-providers.destroy', props.provider.id));
    }
};
</script>
